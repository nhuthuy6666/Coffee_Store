 ...
 */
function mood_enqueue_theme_style( string $handle ): void {
	wp_enqueue_style(
		$handle,
		get_stylesheet_uri(),
		array(),
 ...
 */
function mood_enqueue_admin_custom_assets(): void {
...
function mood_scripts(): void {
	...
	mood_enqueue_theme_style( 'mood-theme-style' );
 ...
 */
function mood_admin_scripts(): void {
	global $pagenow;
	...
	// Enqueue theme style.css (for WordPress theme header).
	mood_enqueue_theme_style( 'mood-admin-style' );

	// Enqueue custom admin CSS/JS.
	mood_enqueue_admin_custom_assets();
}
add_action( 'admin_enqueue_scripts', 'mood_admin_scripts' );
